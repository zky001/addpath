...
MessageBoxA(GetActiveWindow(),buffer,"JPEG Fatal Error",MB_ICONSTOP);
...
// save Jpeg file
BOOL CJpeg::Save(LPCTSTR lpstrFileName, CDib* pDib, BOOL bColor, int nQuality)
{
...
BYTE* CJpeg::ReadJPEGFile(LPCTSTR lpstrFileName, UINT *uWidth, UINT *uHeight)
{
...
char buf[250];
    ...
    //打开JPEG文件
if ((infile = _wfopen(lpstrFileName, _T("rb"))) == NULL)
{
...
BOOL CJpeg::WriteJPEGFile(LPCTSTR lpstrFileName,
BYTE *dataBuf,
UINT widthPix,
UINT height,
BOOL color,
...
jpeg_create_compress(&cinfo);
      ...
      //打开文件并设定数据目标
if ((outfile = _wfopen(lpstrFileName, _T("wb"))) == NULL)
{
